...
public class DownLoadService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		myNotification.showCustomizeNotification(R.drawable.app_logo, getString(R.string.app_name) + "更新包",
				R.layout.down_nofity);

		filePathString = Config.APKPATH + File.separator + getString(R.string.app_name) + ".apk";

		// 开启一个新的线程下载，如果使用Service同步下载，会导致ANR问题，Service本身也会阻塞
		String url = intent.getStringExtra("url");
		if (!TextUtils.isEmpty(url)) {
			downFileThread = new DownFileThread(this, updateHandler, intent.getStringExtra("url"), filePathString);
