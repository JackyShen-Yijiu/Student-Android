...
public class SubjectTwoActivity extends BaseActivity {
	...
	private void initView() {
		...
		findViewById(R.id.main_appointment_layout).setBackgroundColor(Color.parseColor("#ff9900"));
		findViewById(R.id.main_appointment_car_layout).setBackgroundColor(Color.parseColor("#bd1f4a"));
		findViewById(R.id.main_appointment_course_layout).setBackgroundColor(Color.parseColor("#ff6633"));
	...
	private void runIntent(String type) {
		...
		if (type.equals(checkEnrollState_my)) {
			...
			if (app.userVO.getSubject().getSubjectid().equals(SubjectStatu.SUBJECT_TWO.getValue())) {
				Intent intent = new Intent(this, AppointmentCarActivity.class);
				intent.putExtra("subject", "2");
				startActivity(intent);
			} else if (app.userVO.getSubject().getSubjectid().equals(SubjectStatu.SUBJECT_ONE.getValue())) {
				ZProgressHUD.getInstance(this).show();
				ZProgressHUD.getInstance(this).dismissWithSuccess("待学习科目二再预约");
			} else {
				ZProgressHUD.getInstance(this).show();
				ZProgressHUD.getInstance(this).dismissWithSuccess("您已完成该科目,无需预约");
			}
		}
	}
