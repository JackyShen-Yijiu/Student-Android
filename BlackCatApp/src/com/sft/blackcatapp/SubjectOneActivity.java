...
public class SubjectOneActivity extends BaseActivity {
	...
	private void initView() {
		...
		((TextView) findViewById(R.id.main_appointment_tv)).setText("科一" + getString(R.string.question_bank));
		((TextView) findViewById(R.id.main_appointment_car_tv)).setText("科一" + getString(R.string.mock_examination));
	...
	public void onClick(View v) {
		...
		case R.id.main_appointment_layout:
			...
			if (app.questionVO != null) {
				...
				intent.putExtra("url", app.questionVO.getSubjectone().getQuestionlisturl());
			} else {
		...
		case R.id.main_appointment_course_layout:
			...
			if (app.isLogin) {
				if (app.questionVO != null) {
					intent = new Intent(this, QuestionActivity.class);
					intent.putExtra("url", app.questionVO.getSubjectone().getQuestionerrorurl());
				} else {
					ZProgressHUD.getInstance(this).show();
					ZProgressHUD.getInstance(this).dismissWithFailure("暂无题库");
				}
		...
		case R.id.main_appointment_car_layout:
			...
			if (app.questionVO != null) {
				intent = new Intent(this, QuestionActivity.class);
				intent.putExtra("url", app.questionVO.getSubjectone().getQuestiontesturl());
			} else {
				ZProgressHUD.getInstance(this).show();
				ZProgressHUD.getInstance(this).dismissWithFailure("暂无题库");
			}
