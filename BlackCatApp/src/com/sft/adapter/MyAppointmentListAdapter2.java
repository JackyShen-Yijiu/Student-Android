...
public class MyAppointmentListAdapter2 extends BaseExpandableListAdapter {
	...
	public MyAppointmentListAdapter2(Activity context,
			Map<Integer, List<MyAppointmentVO>> mData) {
	...
	private View getItemView(List<MyAppointmentVO> list, int position) {
		...
		} else if (state.equals(AppointmentResult.applying.getValue())) {
			// 待接受1
		...
		String url = item.getCoachid().getHeadportrait().getOriginalpic();
		...
		String schoolName = item.getCoachid().getDriveschoolinfo().getName();
		String trainPlace = item.getTrainfieldlinfo().getFieldname();
		...
		String todayPosition = UTC2LOC.instance.getDate(item.getBegintime(),
				"yyyy-MM-dd");
		String nextPosition = null;
		if (position == list.size() - 1) {
			holder.line.setVisibility(View.VISIBLE);
			holder.splitLine.setVisibility(View.GONE);
			nextPosition = UTC2LOC.instance.getDate(item.getBegintime(),
					"yyyy-MM-dd");
		} else {
		...
		if (CommonUtil.compare_date(format.format(new Date()), todayPosition) == 0) {
			holder.time.setText("今天  "
					+ UTC2LOC.instance.getDate(item.getBegintime(), "HH:mm")
					+ "-"
					+ UTC2LOC.instance.getDate(item.getEndtime(), "HH:mm"));
	...
	// .getBegintime(), "yyyy-MM-dd");
	// }
	// if (CommonUtil.compare_date(format.format(new Date()), todayPosition) ==
	// 0) {
	// holder.time.setText("今天  "
	...
	private void showPop(MyAppointmentVO myAppointmentVO) {
		...
		pop.showAtLocation(context.getWindow().getDecorView(), Gravity.TOP, 0,
				0);
